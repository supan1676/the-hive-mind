...
func _set_stylebox_overrides(panel_stylebox: StyleBox, titlebar_stylebox: StyleBox) -> void:
	...
	# Safety Check: If the box isn't a StyleBoxFlat, we can't compare/tween the "bg_color"
	if cur_panel_stylebox is StyleBoxFlat and cur_panel_stylebox.bg_color == frames.panel_normal.bg_color:
		return
	elif not cur_panel_stylebox is StyleBoxFlat:
		return # Stop execution if using a texture or other stylebox type
	...
	# FIX: Only attempt to animate color properties if the stylebox supports them
	if cur_panel_stylebox is StyleBoxFlat and cur_titlebar_stylebox is StyleBoxFlat:
